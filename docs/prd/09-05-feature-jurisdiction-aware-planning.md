...
- **JRP-US-002** 作为广东用户，我需要缺少市级医保退休参数时只让该能力进入人工确认而继续其他计算；作为四川用户，我需要明确得知地区尚未支持，而不是返回上海结果。
...
- **JRP-FR-013 独立开放**：首期上海、广东按地区分别激活，一个地区失败不影响其他已激活地区；四川延期期间不创建发布记录或活动快照。
...
- **JRP-FR-020 能力级缺口**：广东2030年前缺少统筹地市医保退休累计缴费年限参数时，规划仍成功创建，但`needs_agent=true`、包含`W-MI-LOCAL-YEARS-MISSING`且医保退休年限结论为空；其他已具备权威参数的模块继续计算。四川延期期间为地区级unsupported。
...
- **JRP-NFR-009 局部失败隔离**：单一政策能力缺参不得清空其他模块已计算结果，也不得把广东降级为整个地区不支持。
...
- Task3不读取案例治理状态或36条展示案例；E2E使用本Feature自有夹具和任务2冻结快照。Task4也不得修改planning、conversation或地区发布记录。
...
广东能力级缺参不是HTTP错误：响应沿用现有`needs_agent/questions/warnings`契约，其中warning ID固定为`W-MI-LOCAL-YEARS-MISSING`。不得新增全局日期错误，也不得把广东整体支持起点设置为2030年。
...
- 并行开发预留Task3 migration编号0015；Task4使用0016。两条分支不得各自覆盖`drizzle/meta/_journal.json`，最终按0015→0016串行集成。
...
| 广东医保退休地市年限缺失 | 仅对应能力needs_agent，保留其他结果 | 人工核实地市口径后补充参数 |
...
| 隔离 | 上海、广东；四川unsupported | 规则、参数和结果无串区，四川不回退上海 |
| 能力缺口 | 广东2030年前医保退休参数缺失 | needs_agent+稳定warning，其他模块结果保留 |
...
- **JRP-AC-005** Given已激活广东快照，When计算2030年前场景，Then结果不包含其他地方实体，医保退休缺参时needs_agent但其他模块继续；2030年起使用男30年、女25年。
...
- **JRP-AC-017** Given四川仍为Deferred/Blocked，When用户选择510000，Then返回422 `JURISDICTION_UNSUPPORTED`且不创建plan、不使用上海或广东快照。

## 15. Definition of Done

- JRP-FR-001～020、JRP-NFR-001～009均有实现和测试映射。
- JRP-AC-001～017取得新鲜证据。
...
- 广东能力级needs_agent不会阻断其他模块；四川保持地区级unsupported。
