...
本Feature在任务2提供上海候选快照后，与地区感知规划Feature并行建立来源链、地区、质量、归档和确定性策展机制；完成可恢复备份后物理删除399条原始案例和81条展示案例，保留全部528条测试。两条Feature不存在代码或验收前置关系。

## 2. 目标

- 固定案例审计基线和可复现的保留/删除集合。
- 为原始、展示和回归测试建立稳定来源链、地区、内容哈希和质量元数据。
- 使用任务2验收的上海候选快照验证展示案例中的数字和预期结果。
...
- 首期不采集广东或四川案例，不为CN/GD/SC设置展示配额，也不从案例正文猜测地区。
...
- **CLG-FR-003 展示治理字段**：`showcase_cases`增加地区、明确来源案例UID、候选快照ID、质量分、质量状态、内容哈希、策展时间和策展人；首期117条经来源文件确认后均标记310000并绑定上海候选快照。
...
- **CLG-FR-017 并行边界**：案例治理不得读取地区规划发布记录，不修改planning API、conversation画像或地区激活；任务3不得依赖本Feature的36条案例完成验收。
...
| 来源链 | 500回归、117展示 | 全部解析且地区为310000、快照为任务2上海候选快照 |
...
| 并行隔离 | Task3未完成或未合入 | Task4目标测试与验收不读取planning发布记录或36条以外接口 |
...
- **CLG-AC-015** Given任务3尚未完成或未合入，When独立执行案例治理目标测试和验收，Then不读取planning发布记录且结果与任务3状态无关。

## 15. Definition of Done

- CLG-FR-001～017、CLG-NFR-001～008均有实现和测试映射。
- CLG-AC-001～015取得新鲜证据。
- 归档真实恢复通过后才执行删除，归档和旧备份均保留。
- 最终计数固定为452/36/528，测试和来源链无损。
- 36条案例满足全部质量和覆盖门禁并统一绑定任务2上海候选快照；不设置CN/GD/SC配额。
- 公开接口不泄漏原始案例或归档内容。
...
- 528条完整测试及治理前可恢复归档。
- 可供未来运营和地区化扩展使用的稳定案例资产；任务3不等待本Feature完成。
