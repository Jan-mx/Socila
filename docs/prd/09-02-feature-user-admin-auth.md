...
- **AUTH-NFR-003 限流**：注册每IP每小时最多5次；登录每IP每5分钟最多20次，且每IP/规范化用户名每5分钟最多5次（2026-09-14 UAT修复：次数门槛不变，窗口由15分钟改为5分钟；登录页限流提示精确为「请求频繁，请五分钟后再尝试。」）。
...
> 2026-09-03 用户修订：长度下限从12字节调整为8字节，并增加字母+数字组合规则。

- 最少8个UTF-8字节，最多72个UTF-8字节。
