...
项目已经具备地区树、显式overlay、冲突和不可变快照。CN、上海及广东权威资产已进入仓库并完成首期物化、管理员批准和候选快照重放；四川只有3个已核实缴费基数draft参数，三项正式来源仍缺失并按ADR-0010延期。当前首期持久库状态以验收报告§17为准：CN、上海、广东各有可重放候选快照，四川无快照且保持blocked。
...
- 旧上海published资产及规划行为无漂移，tests/cases/showcase_cases计数不变；首期已创建并重放CN、上海、广东各1个候选PolicySnapshot（数据库记录含创建与重放两组记录），四川为0。
- CN、上海、广东必须在管理员批准和候选快照可重放证据完成后才能标记首期Accepted；该门禁已在验收报告§17通过。四川三项延期缺口不阻塞首期，但必须保持blocked、无候选快照且不得描述为已支持。
