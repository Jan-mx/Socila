...
3. 上海dsl规则JSON（R-500/R-510/R-540）：`generic-api-key`命中——决策表业务字段名（`"key": "calc.subsidy.4050_eligible"`），与`.gitleaksignore`既有5个fingerprint基线为同一批已核实误报；09-05目录迁移（`dsl/ssp_dsl_v1`→`dsl/regions/shanghai_dsl_v1`）后旧fingerprint不再匹配新路径。
