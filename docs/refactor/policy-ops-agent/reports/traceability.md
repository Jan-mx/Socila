...
## 09-07 任务3：地区感知用户规划（JRP，2026-09-07）

| 需求范围 | 实现位置 | 测试路径 | 验收证据 | 状态 |
| --- | --- | --- | --- | --- |
| JRP-FR-001～020、JRP-NFR-001～009、JRP-AC-001～017 | `drizzle/0015_jurisdiction_planning_releases.sql`+`drizzle/meta/_journal.json`、`src/lib/db/schema.ts`（jurisdictionPlanningReleases表+plans留痕列）、`src/lib/engine/orchestrator.ts`（snapshotRulesToDefinitions/snapshotParamsToFlat/orchestrateSnapshot）、`src/server/modules/planning/application/{jurisdiction-compute.use-case.ts,stable-errors.ts,index.ts}`、`src/server/modules/planning/infrastructure/drizzle/jurisdiction-planning-read.repository.ts`、`src/server/modules/publishing/application/jurisdiction-release.use-case.ts`、`src/server/modules/publishing/infrastructure/drizzle/jurisdiction-release.repository.ts`、`src/server/modules/conversation/application/jurisdiction-profile.use-case.ts`、`src/app/api/plan/compute/route.ts`（strict+稳定错误映射）、`src/app/api/chat/route.ts`（画像一致性+客户端jurisdiction剥离）、`src/app/api/conversations/[conversationId]/jurisdiction/route.ts`（用户确认）、`src/app/api/admin/jurisdictions/[code]/release/route.ts`（激活/切换）、`src/lib/validators/plan-input.ts`（strict+jurisdiction_code必填）、`src/lib/ai/{tools.ts,agent.ts,prompts.ts}`（computePlan工具地区契约/候选不升级/提示词）、`src/components/chat/{JurisdictionSelector.tsx,jurisdiction-selector-logic.ts}`、`src/app/(client)/chat/ChatPageClient.tsx`（画像恢复）、`src/components/chat/ChatPanel.tsx`（选择器+请求携带地区）、`dsl/regions/sichuan_dsl_v1/rules/.gitkeep`（空目录可复现修复） | 单元：`src/server/modules/planning/application/__tests__/jurisdiction-compute.use-case.test.ts`（10）、`src/server/modules/conversation/application/__tests__/jurisdiction-profile.use-case.test.ts`（9）、`src/server/modules/publishing/application/__tests__/jurisdiction-release.use-case.test.ts`（8）、`src/lib/validators/__tests__/plan-input-jurisdiction.test.ts`（8）、`src/lib/engine/__tests__/orchestrator-snapshot.test.ts`（5）、`src/lib/ai/__tests__/tools-jurisdiction.test.ts`（9）、`src/components/chat/jurisdiction-selector-logic.test.ts`（3）；集成（test:db）：`src/server/modules/policy/__tests__/jrp-0015-migration.integration.test.ts`（3）、`src/server/modules/planning/application/__tests__/jurisdiction-compute.integration.test.ts`（5） | `docs/refactor/policy-ops-agent/reports/feature-09-05-jurisdiction-planning/acceptance-report.md`；TDD Red：6文件19失败（目标行为缺失）；Green：npm test 58文件/538通过、test:db 21文件/95通过、tsc/eslint/build零error、Auth E2E 10/10、Python 94单元+20集成、Gitleaks完整历史零发现、scan-secrets 691文件零命中、allowlist哨兵3场景全过 | Accepted（本任务验收）；持久库0015/地区激活未执行，需用户单独授权 |
