...
| 09-16 后台政策资产中文可读化（APR） | APR-FR-001～017、APR-NFR-001～005、APR-AC-001～012 | `drizzle/0020_apr_display_names.sql`、`drizzle/meta/_journal.json`、`src/lib/db/schema.ts`（rule_sets.name；params.name/description）、`src/lib/dsl/display-names.ts`、`dsl/regions/{cn,shanghai,guangdong,sichuan}_dsl_v1/rule_sets/*.json`与`params/*.json`（人工中文名称/说明逐项补齐）、`src/lib/db/seed/seed-params.ts`与`seed-misc.ts`（装载入口强制名称）、`src/lib/policy-materialization/manifest.ts`（policyContentOf：显示元数据剥离出内容哈希）、`src/lib/policy-materialization/materialize.ts`（insertEntity写入name/description）、`src/lib/policy-materialization/shapes.ts`（形状本就不含显示字段，增量判定不漂移）、`src/server/modules/rules/domain/rule-set-members.ts`（成员解析纯函数）、`src/server/modules/rules/application/rule-set-view.ts`（精确实体详情/选择器/引用反查）、`src/server/modules/rules/application/ports.ts`与`infrastructure/drizzle/rules-read.repository.ts`（getRuleSetExact/listRuleCandidates批量/listRuleParamReferenceIndex批量）、`src/app/api/admin/rule-sets/route.ts`、`src/app/api/admin/rule-sets/[id]/route.ts`、`src/app/api/admin/rule-sets/[id]/candidates/route.ts`、`src/app/api/admin/params/route.ts`、`src/app/api/admin/publish/pipeline/route.ts`、`src/app/api/admin/publish/history/route.ts`、`src/lib/admin/entity-edit-policy.ts`（name/description白名单）、`src/lib/admin/rule-selector.ts`、`src/lib/admin/publish-history-names.ts`、`src/lib/admin/param-references.ts`、`src/lib/client/asset-display.ts`、`src/app/admin/rules/page.tsx`（删除重复规则集标签页）、`src/app/admin/rule-sets/page.tsx`、`src/app/admin/params/page.tsx`、`src/app/admin/publish/page.tsx`、`src/server/modules/agent-integration/application/materialize.ts`（修复轮I4：新参数草案强制正式名称——zod必填+事务内防线，缺名/空白/HTML非法422且四表零写入，编号回退仅限0020旧行） | `src/lib/dsl/__tests__/display-names.test.ts`、`src/lib/dsl/__tests__/apr-dsl-display-assets.test.ts`、`src/server/modules/rules/domain/__tests__/rule-set-members.test.ts`、`src/lib/admin/__tests__/rule-selector.test.ts`、`src/lib/admin/__tests__/publish-history-names.test.ts`、`src/lib/client/__tests__/asset-display.test.ts`、`src/lib/admin/entity-edit-policy.test.ts`（APR用例）、`src/app/admin/__tests__/apr-admin-ui-contract.test.ts`（源码契约）、`src/lib/db/migration-lf.contract.test.ts`（0020钉入journal单调）、`src/lib/db/__tests__/apr-0020-migration.integration.test.ts`（旧库升级/回退/NOT NULL/幂等）、`src/app/api/admin/__tests__/apr-rule-sets.integration.test.ts`（精确身份/成员顺序/缺失拒绝/选择器/名称更新）、`src/app/api/admin/__tests__/apr-params-publish.integration.test.ts`（参数展示/流水线名称/历史真实性）、`src/server/modules/policy/__tests__/multi-region-seed.integration.test.ts`（夹具名称适配）、`src/server/modules/rules/__tests__/apr-validity-window.integration.test.ts`（修复轮I1有效期窗口6场景）、`src/app/api/admin/__tests__/apr-overlay-carriers.integration.test.ts`（修复轮I2载体成员视图/保存校验/fail-closed）、`src/server/modules/policy/__tests__/apr-snapshot-display-hash.integration.test.ts`（修复轮I3显示元数据与快照哈希隔离）、`src/server/modules/agent-integration/__tests__/materialize-param-draft-name.integration.test.ts`（修复轮I4草案名称零写入）、`src/lib/admin/__tests__/apr-gate-contract.test.ts`（修复轮I6门禁镜像契约）、`src/lib/admin/params-service.test.ts`（修复轮I-B3 display_name检查项）、`services/agent/tests/test_drafting.py`（修复轮I-B6 Agent侧同口径识别）、`e2e/apr-admin-readability.spec.ts`（Chromium E2E；修复轮新增广东overlay分区展开用例与草稿创建幂等） | `docs/refactor/policy-ops-agent/reports/feature-09-16-admin-policy-asset-readability/acceptance-report.md`；PRD：`docs/prd/09-16-feature-admin-policy-asset-readability.md`；分支`codex/apr-policy-asset-readability`（自main `a834d32`，审查基线`3272577`）；修复轮Important×6（I1有效期上界、I2 overlay载体、I3快照哈希、I4 Agent草案name、I5引用跨地区去重、I6门禁MinIO digest）已全部关闭（独立复审A/B：对六项确认关闭；复审B新增I-B1/I-B2/I-B3/I-B6同族缺陷亦已修复，B4/B5及预存在Minor登记于验收报告边界节） | 状态见验收报告§5终态 |
...
| SHV2-NFR-006 / AC-017 同步竞态 | `services/agent/agent/rag/evidence_sync.py`（`_verify_objects_or_conflict`三检查点：ensure后/上传后/提交前） | `tests/test_rag_evidence_sync.py` 85/85（`TestEnsureRaceConflict`、`TestEnsureRaceConflictRealMinio`、`TestPostUploadConflictCheck`、`TestPreCommitObjectCheck`三例RED→GREEN+提交前终检1例（复审P2-1修正注入点，旧实现RED））；drill竞态步骤ok | 已交付（待用户测试） |
...
## 当前交付复核（功能分支tip，实现主体`d32b812`，2026-09-13）
