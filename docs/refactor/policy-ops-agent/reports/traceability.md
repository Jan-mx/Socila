...
| 09-05 上海案例库精简、质量治理与原始数据归档（任务4） | CLG-FR-001～017、CLG-NFR-001～008、CLG-AC-001～015 | `drizzle/0016_clg_case_governance.sql`+journal、`src/lib/db/schema.ts`（cases/showcase_cases/tests治理字段+case_archive_batches/case_archive_entries）、`src/lib/case-governance/`（types/hashes/scoring/eligibility/replay/multi-label/source-chain/region/curation/manifest/archive/reconcile/apply）、`src/lib/showcase/seed-showcase.ts`、`src/lib/import/excel-import.ts`（回归source_case_uid回填）、`scripts/govern-case-library.ts`（audit/prepare-archive/verify-archive/apply/verify）、`src/server/modules/case-governance/`（归档元数据只读应用）、`src/app/api/showcase-cases/route.ts`、`src/app/api/admin/cases/route.ts`、`src/app/api/admin/case-archive/{route.ts,[batchId]/route.ts}`、`src/app/admin/archive/page.tsx`、`dsl/regions/sichuan_dsl_v1/rules/.gitkeep`（空目录占位，修复干净检出） | `src/lib/case-governance/__tests__/`（scoring/eligibility/curation/multi-label/source-chain/region/replay/hashes/manifest/archive/task3-isolation）、`src/server/modules/policy/__tests__/clg-{0016-governance-schema,source-chain-backfill,governance-full-cycle}.integration.test.ts`、`src/app/api/admin/__tests__/clg-admin-api.integration.test.ts`、`src/server/modules/__tests__/module-boundaries.test.ts`（case-governance模块） | `docs/refactor/policy-ops-agent/reports/stage-09-05-case-governance/acceptance-report.md`；分支`codex/task4-case-governance`；提交`feat: 完成上海案例库质量治理`（已推送；SHA见Git历史） | 代码Accepted；持久库apply待单独授权 |
## 当前Work Item
