...
## 16. 分地区交付范围决策（2026-09-07，文档决策时点记录）

本节记录ADR-0010产生的范围决策，不改写§1～§15历史执行事实。以下“未执行”描述均是2026-09-07文档决策时点的边界；后续实际执行结果以§17.8为准。

### 16.1 首期与延期范围

- 任务2首期交付CN、上海310000和广东440000；三地区必须完成权威规则、管理员批准和可重放候选快照。
- 四川510000转Deferred/Blocked：保留现有3个draft参数和3条原因，不批准、不创建候选快照、不开放流量；后续由`WI-20260907-01-sichuan-policy-followup.md`独立验收。
- 四川延期不阻塞任务2首期Accepted，但任何四川请求不得回退到上海或广东。
- （历史决定，已由ADR-0011替代）任务2Accepted后曾计划并行开发任务3/4；独立复审后当前顺序改为任务3修复→地区案例重建→受控持久替换。该调整不影响任务2首期Accepted。

### 16.2 广东能力边界

- 广东整体可作为首期支持地区。2030年前缺少统筹地市医保退休累计缴费年限时，只由R-220输出`needs_agent=true`和`W-MI-LOCAL-YEARS-MISSING`，医保退休年限结论为空；养老、缴费基数、最低工资和失业模块继续计算。
- 2030-01-01起省级统一男30年、女25年进入有效参数集合。
- 广东失业保险金金额公式“领取地市最低工资×0.9”已在任务2实现；任务3只传递用户确认的领取地市。

### 16.3 当前阻断与目标

（历史时点）2026-09-07对当时HEAD执行只读fresh audit：持久库为49/70/5/4/528/851/117/0，audit错误规划74/116/9/8，且广东携带已过期的缴费基数/失业条例blocking reasons；因此当时禁止apply。该阻断已在§17.1～§17.4修复并完成验证。

（历史计划，已完成）任务2实现批次已取得专用Red/Green并将计划收敛为广东delta：5参数、1规则、1规则集新版本、1政策包新版本；CN、上海、四川和未变化GD实体零新增。候选快照前目标业务计数为50/75/6/5/528/851/117/0，随后已完成GD物化、管理员批准和三地区候选快照重放。

## 17. 任务2首期：广东增量物化与地区就绪修复（2026-09-07；17.1～17.7为历史执行批次）

按ADR-0010 §16范围执行首期实现批次：广东确定性delta物化、失业保险金金额规则、地区就绪修正。17.1～17.7记录代码批次及其当时的授权边界；后续§17.8已完成持久库apply、管理员批准、候选快照创建/重放和独立复审，当前最终状态为本报告顶部及§17.8所述Accepted。
...
### 17.7 边界与状态（历史记录；最终状态见§17.8）

- （历史边界）本代码批次未执行持久库apply、管理员批准或候选快照创建；后续受控执行已在§17.8记录。四川实体仍未修改，生产流量仍未开放。
- 在17.1～17.7代码批次结束时，任务2整体确实保持**Reopened**，持久库GD物化、三地区管理员批准和候选快照仍等待授权；这些是历史时点事实，不覆盖后续§17.8的完成证据。

### 17.8 管理员批准与三地区候选快照（2026-09-07，用户指示"三个地区全部为批准并继续"）
