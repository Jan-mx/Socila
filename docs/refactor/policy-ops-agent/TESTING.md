...
- **演练升级**：`scripts/rag-evidence-drill.mjs` 17项→22项——新增守卫反例F codeSha不一致exit4、G dirty工作树exit2、H RAG数据库漂移exit4（外部插入行单独记录）、I write-only权限错误经受限IAM用户（mc创建仅s3:PutObject策略用户）真实AccessDenied exit1失败关闭、bucket创建竞态归属准确步（外部预先建桶→重plan→apply bucketCreated=false且上传/登记/verify正确）；全部守卫反例输出rag.sources/rag.fetches/rag.document_versions规范化行hash+行数与MinIO对象层before/after指纹；22项全ok（证据`rag-evidence-drill-2026-09-12T16-47-00-452Z.json`）。运行前提同上+Docker（minio/mc:latest镜像用于受限用户步骤）。
