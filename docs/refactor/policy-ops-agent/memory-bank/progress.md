...
- 状态：Stage 07 迁移与发布执行中（Stage 01~06 全部验收 PASS，提交 0f63530→27a0a68→fc43368→95ea7ff→d20ca01→1ec7f09 均已推送）。
- 日期：2026-08-30
...
- 07.9 切换准备：**完成，ready-to-release**（`reports/stage-07/cutover-checklist.md`）。含用户授权决策项、切换执行序（预计停写 ≈30min）、阻断冒烟清单、真机补录项。**07.10 为强制暂停点：需用户显式授权后执行。**07.9 复核中补齐 07.1 遗留缺口：新增 `infra/prod/Caddyfile`（默认 :80，`SITE_ADDRESS` 可启自动 HTTPS）并将 proxy 服务加入 edge 网络（此前与 web 不互通，属部署阻断项）；`docker compose config` 校验通过。
