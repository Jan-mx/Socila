...
1. 执行阶段06提交 `feat: 实现政策影响分析与草案审核闭环` 并推送。
2. 进入 Stage 07 迁移与发布（07.1 Compose 与网络起）。
3. 07.4/07.5 两次 Neon 迁移演练、07.7 空机恢复、07.8 回退演练。
4. 07.9/07.10 生产切换需用户显式授权；未授权只交付 ready-to-release。
5. 07.11 最终独立审查 + 07.12 阶段验收 → Goal 完成。
