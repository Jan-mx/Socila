...
| 04.8 建立模型Fake与观测 | AGT-FR-012 | 节点接口 | 确定性模型桩和事件指标 | 重放与日志敏感检查 | 测试无需外部API |
| 04.9 阶段验收 | AGT-AC-001～006 | 04.1～04.8 | 阶段04验收报告 | API/队列/恢复/幂等/权限 | PASS后提交并推送 |
...
| 05.2 接入Docling | RAG-FR-004、007 | 多格式样本 | DocumentTree基础转换 | HTML/PDF/DOCX/MD/TXT/JSON | 结构和资源可回溯 |
| 05.3 接入OCR和人工校对 | RAG-FR-005 | 扫描PDF/图片 | 布局、表格、置信度、校对 | 低置信度暂停恢复 | 未校对内容不索引 |
| 05.4 接入Excel参数表 | RAG-FR-006 | XLS/XLSX样本 | 单元格和表格结构 | 类型/合并/坐标测试 | 参数信息无损 |
...
| 05.7 安全验证SiliconFlow | PRD-FR-024 | 轮换后local key | 模型/路径/维度验证报告 | `/models`/embedding/rerank/异常 | 无秘密输出 |
...
| 05.11 阶段验收 | RAG-AC-001～007 | 05.1～05.10 | 阶段05验收报告 | 格式/OCR/安全/检索/API | PASS后提交并推送 |
...
| 步骤 | 需求 | 输入 | 交付物 | 验证 | 完成条件 |
| --- | --- | --- | --- | --- | --- |
| 07.1 固定Compose与网络 | REL-FR-001～004 | 全服务清单 | 镜像、网络、健康、资源和Secret | 空服务器启动/端口扫描 | 仅代理对外 |
...
| 07.6 容量与安全验收 | REL-FR-001～009 | 生产等价环境 | 容量、安全、Secret报告 | 并发/OCR/索引/漏洞测试 | 无阻断项 |
| 07.7 空服务器恢复 | REL-FR-007～009 | 离机备份与Runbook | 全栈恢复证据 | 用户/规划/Agent可用 | 达到RPO/RTO |
| 07.8 回退演练 | REL-FR-010～012 | 旧应用与Neon只读 | 回退步骤和证据 | 模拟切换失败 | 约定RTO内恢复 |
| 07.9 准备生产切换 | REL-FR-010～012 | 全部验收 | 维护窗口和检查单 | 用户授权门禁 | 未授权保持ready |
| 07.10 执行正式切换 | REL-FR-010～012 | 用户明确授权 | 最终迁移和入口切换 | 数据对账/冒烟/监控 | 成功或按条件回退 |
| 07.11 最终独立审查 | 总体DoD | 所有阶段证据 | 最终验收报告 | 抽样重跑与Git审计 | 无缺失证据 |
| 07.12 阶段验收 | REL-AC-001～007 | 07.1～07.11 | 阶段07验收报告 | 部署/迁移/恢复/回退 | PASS后提交并推送 |
