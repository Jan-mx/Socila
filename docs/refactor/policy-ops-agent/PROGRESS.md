...
| 任务3：地区感知用户规划 | **Accepted（2026-09-07）**：地区感知规划代码与测试已交付并验收；持久库0015已执行、上海/广东地区发布记录已激活（经publishing用例+新鲜管理员），四川保持无发布记录 | 与任务4并行开发；最终按0015→0016串行集成 |
...
| 边界 | 未创建/替换/删除持久快照、未改四川实体（.gitkeep仅恢复空目录结构）、未读任务4案例状态与36条展示案例；演练容器`jrp-drill-pg`（5439）与卷`jrp-drill-pg-data`保留，`nrp-drill-pg`已停止保留 |
| 持久库执行（用户授权"自动全部批准执行并继续"） | 备份`policyops-jrp-pre-20260907184211.dump`（SHA-256 `4cf4b86a…`）+全新PG17真实恢复37表+18 sequence对账一致；0015迁移×2幂等（journal 14→15）；经`activateJurisdictionRelease`+`requireFreshAdmin`激活310000（快照`c9ca7079…`）与440000（快照`77781869…`），四川0条；重复激活幂等；persist=false规划验证（沪24规则/粤17规则快照成员执行）；激活后备份`policyops-jrp-post-20260907184612.dump`（SHA-256 `f14b6620…`）+37表+19 sequence恢复对账一致；演练恢复容器已清理 |
