...
09-11 Feature（`docs/prd/09-11-feature-shanghai-case-library-v2.md`，上海政策纠偏与36条案例V2重建）在功能分支`codex/shanghai-case-v2`实施完成（**未合并目标分支，等待用户测试**）：WI-20260911-01上海官方证据与政策纠偏、WI-20260911-02 RCL-GEN-2.0案例/Markdown案例库/API/UI、WI-20260911-03 0019审计迁移与受控原位改写CLI均**Accepted（2026-09-11）**——36条V2案例（数值单源、完整policySources、快照绑定与content hash）、案例库Markdown+manifest（`--check`逐字节对账）、公开/管理API新增`caseNature`/`policySources`（向后兼容）、公开页与后台完成合成披露文案替换、0019纯审计迁移与受控原位改写（守卫/单事务/幂等/并发/post dump恢复对账全部隔离验证）。全部门禁本地新鲜通过（单元840+零skip、隔离库test:db 156零skip、Chromium 23/23含V2终态分支、Gitleaks全历史98提交经精确allowlist零发现），持久policyops全程未连接未写入；对持久库的0019与改写执行为PRD §18授权点，须另行fresh授权。
