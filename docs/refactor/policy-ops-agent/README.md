...
09-05 Stage（`docs/prd/09-05-stage-national-baseline-regional-overlays.md`）按ADR-0010完成**任务2首期CN、上海、广东分地区交付（Accepted，2026-09-07）**。任务3（WI-20260907-02）保持**Accepted**。**任务4（WI-20260907-03）第四轮修复完成、重新Reopened（2026-09-10）**：第三轮修复（旧test完整hash、manifest三方自校验、SHA清单精确覆盖、restore真实验证、selection真实计算、42 example原子同步、批次状态0行失败、落库行hash逐项核对）全部保持；**第四轮修复4项复审缺陷**——①prepare-archive补偿：任何prepare阶段失败不留prepared批次/entries、只精确清理本次batchId（status='prepared'守卫）、文件失败清理本次不完整归档、补偿失败同时报告原始错误与补偿错误（`RclPrepareError`）；②applied幂等重验：`executeRclApply`对applied批次先完整重验manifest正文hash、批次hash、最终N/36/N+42、42条example与cases/showcase/regression逐行hash，完全一致才noop，任一最终行缺失/增加/漂移返回稳定错误且零写入；③migration换行契约：仓库根`.gitattributes`固定`drizzle/*.sql text eol=lf`（0010～0018 blob不变），Windows `core.autocrlf=true`下全新checkout仍为LF，Drizzle读取hash===Git blob LF SHA；④迁移审计语义：审计同时输出Git blob SHA/工作树raw SHA/LF规范化SHA/CRLF规范化SHA/账本SHA/仅EOL差异/真实内容差异，并同步核对journal与账本created_at严格单调（不符仅报告，禁止猜测修复）。门禁：`npm test` 74文件/713零skip；`npm run test:db` 26文件/141零skip（随机端口全新PG17+pgvector）；tsc/eslint/build零error；pytest 94+20（integration 20/20）；Gitleaks 8.29.1完整历史85提交零发现；scan-secrets 788文件零命中；allowlist哨兵全过；Markdown相对链接与`git diff --check`通过。**第四轮只读审计与repair-forward**：当前库36/36/78冻结；可信旧归档永久保存于`F:/Socila/backup/case-library/task34-r4-trusted-old-2026-09-10T06-59-14/`（manifestHash `da0ea94d…`，452/36/500全逐行ID/UID/64位hash、500 test hash全部非空、40表+20 sequence真实restore明细、sha256sums恰好7文件、第三库二次对账一致）；当前36/36/78 attestation绑定代码提交`579fed8…`（attestationManifestHash `eb6d8d9d…`、targetFingerprint `56c479de…`）；迁移换行审计确认ID 10/11/12/13/14/15/21/22账本hash===0010～0018 Git LF内容、ID 18/19/20===0012/0013/0014 CRLF重复登记、ID 17缺号不补写；repair-forward计划（codeSha `579fed8…`、trustedArchiveManifestHash `da0ea94d…`、trustedArchiveDumpSha `0e3c3d8b…`、migrationLedgerFingerprint `492c5fbe…`、精确SQL写集合10步、前置条件/事务边界/回退点/失败条件）见`F:/Socila/backup/case-library/task34-r4-audit-2026-09-10T06-59-14/repair-forward-plan.json`。**WI-20260907-04保持Reopened（等待repair-forward授权）**；`refactor/policy-ops-agent-platform`仍停在`57f051d`，不得执行最终合并、重跑旧stage脚本或自动恢复pre dump。
