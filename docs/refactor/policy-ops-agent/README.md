...
09-16 Feature（`docs/prd/09-16-feature-admin-policy-asset-readability.md`，后台政策资产中文可读化APR）**Accepted（2026-09-17：用户UAT通过，PR#2 squash合入main 3f387db；持久库0019+0020已执行且行数零变化，web/agent/worker/beat已滚动替换并健康冒烟通过）**：基线`3272577`曾记录"四轮复审Important=0"，已被后续独立复审取代——复审Important×6与复审再发现I-B1/B2/B3/B6均经RED→GREEN修复，最终HEAD新鲜门禁全绿（单元104文件/1057用例、drill `test:db` 36文件/198用例0skip、pytest integration 131/0skip、tsc/eslint/build通过、APR Chromium E2E 5/5）且复审终局Critical=0、Important=0。修复内容：基线`3272577`曾交付代码并记录"四轮独立复审Critical=0、Important=0/Accepted"，该结论已被后续独立复审取代——复审发现Important×6：①`listRuleCandidates`有效期上界方向错误（`as_of>=effective_to`选中过期并排除仍有效行）；②规则集成员解析与保存校验只按成员编号装载，遗漏非成员overlay载体（如广东`R-GD-MI-RETIRE-RESTRICT`指向`R-220-MEDICAL-LIFETIME-GAP`），页面误显示纯CN baseline；③政策快照`payload`写入完整Drizzle行，参数`name/description`与规则集`name`进入`contentHash`，违反APR-NFR-004；④Agent新参数草案`name`可选并回退编号，违反APR-FR-017；⑤参数引用反查仅按`ruleId`去重，CN与地区同编号互相覆盖；⑥APR数据库门禁使用不可拉取且非确定性的`minio/minio:latest`。修复以RED→GREEN逐项补齐判别力测试，完成前不预写通过结论。以下为基线交付范围（历史记录）：规则管理页仅删除重复的"规则集"标签页且展示行为不变，`/admin/rule-sets`为唯一规则集入口（精确实体身份`rule_set_id+jurisdiction_code+version`，成员按持久化顺序展示继承来源地区/版本/overlay操作、可展开只读成员内容、名称/编号搜索选择器）；规则集、参数与规则获得正式中文`name`（0020迁移+四地区DSL人工名称逐项补全；显示元数据经`policyContentOf`剥离出contentHash/payloadShape，旧库升级物化与快照零漂移）；参数页名称为主并批量反查引用规则（按地区链过滤）；发布中心按rule_set→rule→param固定顺序分组、卡片显示精确版本名称、历史无法精确解析时显示"名称不可用"不猜测。规则引擎、执行顺序、参数值、快照、发布门禁、回滚、overlay与历史重放语义零变化（黄金回归+契约测试）。新鲜门禁：`npm test` 103文件/1037用例skip 0、drill `test:db` 32文件/176用例skip 0、agent.migrate幂等+pytest integration 131、tsc/`eslint src e2e --max-warnings 0`/build通过、Chromium E2E 4/4；证据见`reports/feature-09-16-admin-policy-asset-readability/acceptance-report.md`。
