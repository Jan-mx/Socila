...
09-11 Feature（`docs/prd/09-11-feature-shanghai-case-library-v2.md`，上海政策纠偏与36条案例V2重建）已在功能分支`codex/shanghai-case-v2@f583adc`交付代码与隔离证据，但2026-09-12独立审查后**Reopened（未合并目标分支）**：①23份上海原件仅在Git evidence目录，尚未闭环MinIO `policy-originals/originals/<sha256>`与RAG `object_key`；②V2 rewrite审计hash存在，但业务表`cases/showcase_cases.content_hash`未证明同步；③完整`npm test`为842/843，migration当前工作树用例5秒超时（单文件7/7通过不能替代完整门禁）。修复与独立复审完成前不得交付用户最终测试或申请持久授权；持久policyops、政策物化、快照/release、0019与案例改写均未执行。
