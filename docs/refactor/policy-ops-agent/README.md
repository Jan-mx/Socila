...
09-05 Feature（`docs/prd/09-05-feature-socila-naming-regional-dsl.md`，Socila命名统一与地区DSL分层）**Accepted（2026-09-05复审纠正后重新验收）**：复审三项缺漏已纠正——命名契约区分"允许的精确旧协议片段"与"独立品牌标识"（`npm test` 359/359全绿）、`.gitleaks.toml`改用`[[allowlists]]`+`targetRules`并新增哨兵回归（ADR-0009，不再整文件跳过）、多地区Seed补齐jurisdiction作用域并有落库级测试（test:db 52通过）：通用协议`dsl/protocol/socila_dsl_v1`与上海地区`dsl/regions/shanghai_dsl_v1`分层，规则格式唯一规范值`SOCILA-DSL-1.0`，Seed经地区Manifest发现（jurisdiction_code=310000由Manifest声明）；活动代码与配置完成SSP/SSRP→Socila一次性硬切换（无旧环境变量、Cookie、localStorage或服务身份兼容）；Node与Python服务JWT身份原子切换为`socila-next-core`；粤川示例转测试夹具，生产Seed不再写入；0010迁移完成dsl_version规范化与持久库六条示例精确清理（备份+真实恢复对账后执行）。证据见`reports/feature-09-05-socila-naming/acceptance-report.md`。
