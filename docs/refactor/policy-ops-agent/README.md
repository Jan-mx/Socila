...
09-05 Stage（`docs/prd/09-05-stage-national-baseline-regional-overlays.md`）按ADR-0010完成**任务2首期CN、上海、广东分地区交付（Accepted，2026-09-07）**。任务3（WI-20260907-02）保持**Accepted**。任务4与WI-20260907-04经第三轮复审重新**Reopened**：旧500 regression归档hash为空、恢复报告表/sequence明细为空、applied manifest声明85 tests而持久库实际78，迁移账本21条且0012～0014重复登记。当前持久库已是36 cases/36 showcase/42 example+36 regression/10 snapshots，暂时保留并冻结写入。顺序改为任务4代码修复→当前库只读取证→新授权repair-forward→WI-04重新Accepted→WI-20260909-01最终合并；`refactor/policy-ops-agent-platform`仍停在`57f051d`，不得执行最终合并、重跑旧stage脚本或自动恢复pre dump。
