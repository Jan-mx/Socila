...
09-11 Feature（`docs/prd/09-11-feature-shanghai-case-library-v2.md`，上海政策纠偏与36条案例V2重建）在功能分支`codex/shanghai-case-v2`实施中（**不自动合入目标分支**）：WI-20260911-01上海官方证据与政策纠偏**Accepted**；WI-20260911-02 RCL-GEN-2.0案例/Markdown案例库/API/UI**Accepted（2026-09-11）**——36条V2案例（沪粤18/18、§8.2矩阵、数值单源、完整policySources、快照绑定与content hash）、案例库Markdown+manifest（`--check`逐字节对账）、公开/管理API新增`caseNature`/`policySources`（向后兼容）、公开页与后台完成合成披露文案替换；全部门禁本地新鲜通过（Node 829零skip、隔离库test:db 144零skip、Chromium 23/23、Gitleaks完整历史96提交零发现），持久policyops未连接未写入。WI-20260911-03（0019审计迁移与受控原位改写）待实施。
