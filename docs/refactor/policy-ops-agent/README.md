...
09-11 Feature（`docs/prd/09-11-feature-shanghai-case-library-v2.md`，上海政策纠偏与36条案例V2重建）当前为**Updating / Reopened**：历史四轮修复保留，但运行时RAG仍未闭环。生产MinIO bucket=0，RAG七张表=0；`evidence_sync`只登记到`downloaded`，没有DocumentTree/chunks/embeddings；Web对话没有`searchPolicy`，不会返回RAG命中原件链接。独立复审另发现ensure期间外部写入冲突对象可能在事务外verify前留下RAG登记。后续由`WI-20260913-01-shanghai-rag-runtime-closure.md`实施；用户人工测试前不合并目标分支，生产fresh授权前不写当前MinIO或policyops。
