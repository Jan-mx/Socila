...
- **draft包repair边界（WI-20260906-01）**：audit输出`packSnapshotDrift`，但repair不得仅依赖业务计数和published哈希；目标指纹必须绑定目标draft包行ID、地区、版本、状态、快照哈希及成员哈希。repair在单事务内锁定并重新校验全部目标，原物化批次/成员不可改写；修复以确定性`repaired`批次和新成员追加审计，沿用地区readiness与blocking reasons，并由0014唯一约束保证并发单结果。该Work Item及专用数据库集成测试Accepted前，持久库repair保持禁止。
