...
- **地区化合成案例（ADR-0011，第三轮复审Reopened）**：生成器、完整场景字段和36/18/18展示路径可复用，但受控归档边界未闭环：旧regression test未计算完整内容hash，restore/SHA/selection只做表面验证，42条example未进入同一manifest/apply事务，applied manifest与当前36/36/78不一致。当前数据冻结；后续执行器必须以完整旧行hash、精确文件清单、真实表/sequence恢复明细、manifest自校验和原子example同步建立可信事实链，再经独立repair-forward验收。
