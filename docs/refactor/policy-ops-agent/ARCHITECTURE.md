...
- **日期快照调度（ADR-0011，已实现）**：任务3按地区和`as_of_date`选择唯一active快照区间（0017`effective_from/effective_to`+同地区active不重叠EXCLUDE约束）；缺失或无匹配区间fail-closed（409`POLICY_SNAPSHOT_UNAVAILABLE`）。发布必须真实执行七道门禁（引用/Schema/参数依赖/冲突/黄金测试/双重重放/成员规范化hash，JRP-FR-007），执行期每次重算成员hash并核对完整gateResults（JRP-FR-026）；历史plan按保存的snapshot ID/hash/date经`POST /api/plan/:id/replay`重放并返回漂移结论。领取地市对外只接受六位行政代码（`profile.claim_city_code`），服务端确认属于广东启用地级市后转换为规则内部`claim_city`规范名称；未知/跨省/未确认城市不估算失业金额。新会话先经`POST /api/conversations`认证预创建再允许地区确认（JRP-FR-021）。
- **地区化合成案例（ADR-0011，已实现）**：任务4基于任务3日期快照生成上海/广东政策案例（`src/lib/case-governance/generator.ts` RCL-GEN-1.0）——场景模板、显式断言（至少1条可比，无可比断言不得分）、覆盖义务、快照hash与证据构成事实链；不使用LLM或真实用户转录。归档清单对真实文件字节计算SHA-256（`archive.ts`），包含完整库dump、三表dump、selection/manifest/restore报告与不自包含的sha256sums.txt；restore报告绑定来源dump SHA、PG/pgvector版本、全部表/sequence与规范化哈希（`reconcile.ts`）。替换manifest绑定新旧行ID/内容hash/快照/评分/来源（`manifest.ts`）；apply使用批次行`FOR UPDATE`、`restore_verified→applying→applied`状态机与归档条目唯一约束并发裁决（`apply.ts`）。质量总分与逐项分解落库（`scoring.ts`），多标签同时保留（`multi-label.ts`）；管理查询`active AND filters`。旧案例只进入离线可恢复归档，运行库最终计数由覆盖manifest的`N/36/N+42`决定（0018迁移，0016哈希不变）。
