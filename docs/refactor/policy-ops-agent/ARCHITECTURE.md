...
- **地区化合成案例（ADR-0011，已实现；2026-09-09第二轮修复）**：任务4基于任务3日期快照生成上海/广东政策案例（`src/lib/case-governance/generator.ts` RCL-GEN-1.0）——场景模板、显式断言（至少1条可比，无可比断言不得分）、覆盖义务、快照hash与证据构成事实链；不使用LLM或真实用户转录。**受控执行器（RCL-FR-021）**：`scripts/rcl-case-library.ts`七模式（audit/generate/plan-replacement/prepare-archive/verify-archive/apply/verify）调用`src/lib/case-governance/executor.ts`真实业务逻辑并输出可验证JSON、按失败原因返回非零退出码；归档清单对真实文件字节计算SHA-256（`archive.ts`+`bufferSha256`，String(buffer)有损解码已修复），包含完整库dump、三表dump、selection/manifest/restore报告与最后生成的不自包含sha256sums.txt；restore报告绑定来源dump SHA、PG/pgvector版本、全部表/sequence与规范化哈希（`reconcile.ts`）。**完整manifest（RCL-FR-006/018）**：新行绑定scenarioKey/asOfDate/input/expected/assertions/coverage/evidence/质量分解/多标签/快照，0018追加cases.input/expected/assertions列；apply在事务内`FOR UPDATE`+`restore_verified→applying→applied`状态机+归档条目唯一约束并发裁决，`assertCompleteScenarioFields`对空占位fail-closed（零写入），行内容hash在plan/apply两侧统一按原生SQL行重算（库中content_hash列可为空，RCL-AC-003）。激活门禁的golden_tests只加载source='example'的DSL示例（RCL-FR-007）。管理查询`active AND filters`。旧案例只进入离线可恢复归档，运行库最终计数由覆盖manifest的`N/36/N+42`决定（0018迁移，0016哈希不变）。
