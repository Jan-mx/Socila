 ...
 * - SOCILA_E2E_DATABASE_URL        验收库连接串（必须存在，且指向本地库）
 * - SOCILA_E2E_PORT / SOCILA_E2E_MOCK_PORT  本地端口
 * - SOCILA_E2E_NEXTAUTH_SECRET / SOCILA_E2E_REFRESH_PEPPER（必须非空且两者不同，§12.2）
 *
...
const databaseUrl = process.env.SOCILA_E2E_DATABASE_URL;
if (!databaseUrl) {
  fail("SOCILA_E2E_DATABASE_URL 未设置：请指向已迁移/引导/seed 的全新 PostgreSQL 17 验收库");
}
try {
  const host = new URL(databaseUrl).hostname;
  if (!["localhost", "127.0.0.1", "::1"].includes(host)) {
    fail(`SOCILA_E2E_DATABASE_URL 指向非本机主机 (${host})：E2E 只允许本地全新库`);
  }
} catch {
  fail("SOCILA_E2E_DATABASE_URL 不是合法连接串");
}

// PMG-FR-003 / PMG-AC-008：NEXTAUTH_SECRET 与 AUTH_REFRESH_PEPPER 必须非空且不相等。
const nextauthSecret = process.env.SOCILA_E2E_NEXTAUTH_SECRET;
const refreshPepper = process.env.SOCILA_E2E_REFRESH_PEPPER ?? "";
if (!nextauthSecret || !refreshPepper) {
  fail("SOCILA_E2E_NEXTAUTH_SECRET 与 SOCILA_E2E_REFRESH_PEPPER 均不得为空");
}
if (nextauthSecret === refreshPepper) {
  fail("SOCILA_E2E_NEXTAUTH_SECRET 与 SOCILA_E2E_REFRESH_PEPPER 必须为不同值（§12.2）");
}
...
const port = process.env.SOCILA_E2E_PORT || "3100";
const mockPort = process.env.SOCILA_E2E_MOCK_PORT || "8787";
...
const result = spawnSync(
  ...
  ],
  {
    ...
    stdio: "inherit",
      env: {
        ...process.env,
        SOCILA_E2E_PORT: port,
        SOCILA_E2E_MOCK_PORT: mockPort,
      },
  },
);
