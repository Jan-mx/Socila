...
function makeComputeDeps() {
  return {
    ...
    getActiveRelease: (code: string) =>
      new DrizzleJurisdictionPlanningReadRepository().getActiveRelease(code),
...
describe("地区感知规划端到端（JRP-AC-004/005/017）", () => {
  ...
  it("活动快照切换后历史plan仍使用原snapshotId（JRP-AC-008/NFR-006）", async () => {
    const firstSnapshot = await activate("310000", "2026-09-02");
    ...
    // 切换活动快照（新 asOf 创建并激活）。
    const secondSnapshot = await activate("310000", "2026-09-03");
