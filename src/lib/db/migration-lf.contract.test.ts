...
describe("migration journal严格单调契约（WI-20260907-03第四轮复审修复）", () => {
  ...
  /** 0010～0020预期when（0010～0018必须与持久账本ID 10～16、21、22的created_at一致；
   * 0019为WI-20260911-03新增审计迁移、0020为APR显示元数据迁移，
   * 两者对持久policyops的执行均须另行fresh授权）。 */
  const EXPECTED_TIMES: Record<string, number> = {
    ...
    "0019": 1788797000000, "0020": 1788883200000,
  };
  ...
  it("0010～0018在持久账本（max=0018）上不重应用；journal max恰为0020", () => {
    const journal = JSON.parse(readFileSync(JOURNAL_PATH, "utf8")) as {
      entries: Array<{ tag: string; when: number }>;
    };
    const byTag: Record<string, number> = {};
    ...
    //（旧journal的0014=1788991200000即Re-apply事故）；0019=1788797000000与
    // 0020=1788883200000是高于账本max的新迁移（WI-20260911-03审计结构与APR显示元数据），
    // 对持久policyops的执行必须另行fresh授权。
    ...
    expect(Math.max(...Object.values(byTag))).toBe(1788883200000);
  });
});
