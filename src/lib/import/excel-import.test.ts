...
describe("Excel import mapping", () => {
  ...
  it("maps current and legacy regression headers with boolean expected results", async () => {
    ...
    expect(parseRegressionTestRows([
      ...
      { name: "legacy", source_case_uid: "legacy-case", input: "{\"age\": 60}", expected: "{\"needs_agent\":false}" },
    ])).toMatchObject([
      { name: "current", sourceCaseUid: "case-04", input: { city: "Shanghai" }, expected: { needs_agent: true } },
      { name: "legacy", input: { age: 60 }, expected: { needs_agent: false } },
    ]);
  });
