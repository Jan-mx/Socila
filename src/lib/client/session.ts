 ...
 * 客户端会话标识（兼容用途）。
 *
 * 服务端的权威匿名会话是 HttpOnly cookie `socila-anon-session`（见
 * `src/lib/security/anon-session.ts`），由服务端在首个请求时下发，前端不可见、也无需感知。
 *
 * 这里的 localStorage `socila-session-id` 仅作**向后兼容**：在 cookie 机制上线前创建的会话，
 ...
 *
 * 09-05 SDL-FR-007：键名硬切换为 `socila-session-id`，旧键名不再读取或写入。
 *
 * 之前这段逻辑在 `ChatPanel.tsx` 与 `ChatPageClient.tsx` 里各复制了一份，这里收敛为单一实现。
 */

const LEGACY_SESSION_KEY = "socila-session-id";
