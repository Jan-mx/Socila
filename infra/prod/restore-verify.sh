...
docker rm -f socila-restore-verify >/dev/null 2>&1 || true
docker run -d --rm --name socila-restore-verify -e POSTGRES_PASSWORD=verify \
  pgvector/pgvector:pg17 >/dev/null
until docker exec socila-restore-verify pg_isready -U postgres >/dev/null 2>&1; do sleep 1; done
docker exec -i socila-restore-verify psql -U postgres -c "CREATE DATABASE \"$VERIFY_DB\"" >/dev/null
cat "$DUMP" | docker exec -i socila-restore-verify pg_restore -U postgres -d "$VERIFY_DB" \
...
docker exec socila-restore-verify psql -U postgres -d "$VERIFY_DB" -tAc "
...
PSQL="docker exec socila-restore-verify psql -U postgres -d $VERIFY_DB -tAc"
...
docker stop socila-restore-verify >/dev/null
...
if [ "$BACKUP_OBJ" -gt 0 ]; then
  ...
  docker rm -f socila-restore-minio >/dev/null 2>&1 || true
  docker run -d --rm --name socila-restore-minio --network restore-verify-net \
    -e MINIO_ROOT_USER=verify -e MINIO_ROOT_PASSWORD=verify123 \
    minio/minio:RELEASE.2025-09-07T16-13-09Z server /data >/dev/null
  until docker exec socila-restore-minio mc ready local >/dev/null 2>&1; do sleep 1; done
  docker run --rm --network restore-verify-net -v "$MINIO_DIR:/backup:ro" \
    -e "MC_HOST_dst=http://verify:verify123@socila-restore-minio:9000" \
    minio/mc mirror --overwrite --quiet /backup dst/
  echo "[restore-verify] MinIO 对象：备份文件 $BACKUP_OBJ（mirror 到全新 MinIO 实例成功）"
  docker rm -f socila-restore-minio >/dev/null
