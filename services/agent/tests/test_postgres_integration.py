...
`python -m agent.migrate --with-roles` 的演练库。
未设置时跳过。
"""
...
@pytest.mark.skipif(
    not (DRILL and os.environ.get("AGENT_APP_DB_PASSWORD")),
    reason="requires AGENT_DATABASE_URL/DATABASE_URL + AGENT_APP_DB_PASSWORD",
)
def test_agent_schema_migration_and_role_isolation():
    """AGT-AC-004：agent_app 角色可读写 agent schema，但访问 core 表被数据库拒绝。"""
    import psycopg

    agent_url = "postgresql://agent_app:{}@localhost:5433/drill01".format(
        os.environ["AGENT_APP_DB_PASSWORD"]
    )
