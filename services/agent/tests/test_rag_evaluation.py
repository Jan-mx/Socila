...
语料：沪（养老/失业）、粤（最低工资）、川（医保）示例文档， Fake 向量。
"""
...
from agent.rag.siliconflow import FakeSiliconFlowClient, SiliconFlowClient
...
def _load_real_key() -> str | None:
    ...
    env_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "..", "docs/refactor/policy-ops-agent/config/siliconflow.local.env"
    )
    if not os.path.exists(env_path):
        return None
    for line in io.open(env_path, encoding="utf-8"):
        m = re.match(r"SILICONFLOW_API_KEY\s*=\s*(.+)", line)
...
class TestRagGoldenGates:
    ...
    def services(self, monkeypatch):
        ...
        with connect(DRILL, autocommit=True) as conn:
            ...
            for jurisdiction, url, _ in SAMPLES:
        ...
        if not real_key:
            pytest.skip("requires rotated SILICONFLOW key for real-model evaluation")
        client = SiliconFlowClient(api_key=real_key)

        store = InMemoryObjectStore()
        ingest = IngestService(DRILL, store, set(), client)
        retrieval = RetrievalService(DRILL, client)

        for jurisdiction, url, content in SAMPLES:
            ...
            source_id = connect(DRILL, autocommit=True).execute(
    ...
    def test_golden_gates(self, services):
        ...
        assert metrics["wrong_jurisdiction_count"] == 0
